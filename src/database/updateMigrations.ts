...
${files.map((fileName) => `import { ${className(fileName)} } from './migrations/${moduleName(fileName)}';`).join('\n')}
...
`;
  ...
  console.log('Migrations definitions in src/database/migrations.ts were updated!');
};
